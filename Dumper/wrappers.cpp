...
{
	if (IsUE5) {
		uint16_t header = Read<uint16_t>(object);
		bool     bIsWide = (header & 1) != 0;
		uint16_t strLen  = header >> 1;
		if (strLen >= len) strLen = len - 1;
		if (bIsWide) {
			for (uint16_t i = 0; i < strLen; i++) {
				uint16_t wc = Read<uint16_t>(object + 2 + i * 2);
				buf[i] = (char)(wc < 128 ? wc : '?');
			}
			buf[strLen] = '\0';
		} else {
			Read(object + 2, buf, strLen);
			buf[strLen] = '\0';
		}
		return;
	}
...
std::string UE_FName::GetName() const
{
	...
	uint32_t number;
	if (IsUE5) {
		number = Read<uint32_t>(object + 0x4);
...
{
	return Read<short>(object + defs.UProperty.ElementSize);
}
	...
	for (auto prop = object.GetChildren().Cast<UE_UProperty>(); prop; prop = prop.GetNext().Cast<UE_UProperty>())
	{
		if (prop.IsA<UE_UFunction>())
		{
			...
			for (auto param = fn.GetChildren().Cast<UE_UProperty>(); param; param = param.GetNext().Cast<UE_UProperty>())
			{
				...
				if (flags & 0x400) // ReturnParm
				{
					f.CppName = param.GetType().second + " " + fn.GetName();
				}
				else if (flags & 0x80) // Parm
				{
					bool isOut    = (flags & 0x100) != 0; // OutParm
					bool isConst  = (flags & 0x4)   != 0; // ConstParm
					auto typeName = param.GetType().second;
					std::string prefix;
					if (isConst) prefix += "const ";
					if (param.GetArrayDim() > 1)
					{
						f.Params += fmt::format("{}{}* {}, ", prefix, typeName, param.GetName());
					}
					else if (isOut)
					{
						f.Params += fmt::format("{}{}& {}, ", prefix, typeName, param.GetName());
